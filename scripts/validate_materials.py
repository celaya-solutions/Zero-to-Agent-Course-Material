...
def main():
    ...
    authored=[ROOT/'README.md',*sorted((ROOT/'preparation').glob('*.md')),*sorted((COURSE/'level-01').glob('*.md')),*sorted((COURSE/'level-02').rglob('*.md')),*sorted((COURSE/'level-03').rglob('*.md')),*sorted((COURSE/'level-04').rglob('*.md')),*sorted((COURSE/'level-05').rglob('*.md')),ROOT/'release/level-05-verification.md',ROOT/'projects/front-desk/README.md',ROOT/'release/level-04-verification.md',ROOT/'projects/local-models/README.md',ROOT/'release/level-03-verification.md',ROOT/'projects/watchman/README.md',ROOT/'release/level-02-verification.md']
    ...
    for name in ['preparation','manual-edit','answer-key']:
        if not (COURSE/'level-05'/f'{name}.pdf').exists():errors.append(f'Level 5 missing {name}.pdf')
    ...
    for p in tracked:
        if p.startswith(('.zta/','.env','platform/')) or Path(p).name in {'PROJECT-LAB-01.md','PROJECT-LAB-02.md','PROJECT-LAB-03.md','PROJECT-LAB-04.md','PROJECT-LAB-05.md'}:errors.append(f'Private/generated learner data tracked: {p}')
