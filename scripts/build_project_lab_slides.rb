...
DECKS = {
  ...
  4 => {
    routes: "<p>Own prepared computer, course fork, and local gemma3:4b. Complete preparation before class. Saved evidence stays labeled.</p>",
    anchors: "<p>Sell the outcome, not the robot. Fences, not promises. Vende el resultado. Barreras, no promesas.</p>",
    idea: "<p>A visitor note can look like an owner order. Prompt injection is following that note as an instruction. Outside controls limit access.</p>",
    fence: "<p>Supplied public and fictional records only. Your own local target. Permission before a partner test. No sending or booking tools.</p>",
    demo: "<p>Read C1 and C2 expectations. Show a source miss. Trace fake bait Before, then its removal from a locked request. Pause and inspect zero rows.</p>",
    task1: "<p>Read the brief and FAQ. Write trigger, steps, checks, failure routes, access list, and never list in the private worksheet.</p>",
    task1_check: "<p>One supported fact, one missing fact, and a clear human handoff. No price, callback promise, private contact, or high-stakes advice.</p>",
    task2: "<p>Run five callers. Change one Source rule with the manual card. Run again and compare C1/C2. Build the five-field River message card.</p>",
    task2_check: "<p>Expected, actual, source or boundary, verdict. Keep both receipts. Alias, public route, need, urgency, preferred time; no timing promise.</p>",
    task3: "<p>Run A1-A10 Before. Score all rows. Apply four defenses. Rerun the same ten and then normal callers. Pause; attempt a set; keep the zero-row receipt.</p>",
    task3_check: "<p>Same prompts and model digest. Honest leak and partial counts. Failed and truncated are separate. Four locations; actual or labeled saved stop proof.</p>",
    extension: "<p>Trace one locked payload and private log. Test in-flight pause with a mock. Keep existing controls and fixed tests; do not host a public target.</p>",
    outage: "<p>After two tries or five minutes, use the full saved caller/attack/control packet. Mark every route. A saved switch is not a live click.</p>",
    partner: "<p>Review two existing rows on the learner-controlled screen, with consent. No unused attacks are required. Self-review is valid.</p>",
    spanish: "<p>Define seis partes. Prueba cinco llamadas. Cambia una regla. Compara diez ataques antes y después. Pausa, guarda y presenta.</p>",
    next: "<p>Give the sixty-second outcome pitch. Finish PROJECT-LAB-04.md privately. Upload to Level 4 and reopen it, or mark Not submitted.</p>",
