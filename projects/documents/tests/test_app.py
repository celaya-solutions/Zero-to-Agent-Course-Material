...
def test_load_index_predictions_and_refresh(tmp_path,monkeypatch):
    ...
    app=AppTest.from_file(str(root()/'projects/documents/src/zta/app.py'),default_timeout=20).run()
    ...
    fresh=AppTest.from_file(str(root()/'projects/documents/src/zta/app.py'),default_timeout=20).run()
